...
class NERTool(Tool):
    ...
    def get_score(self, model, x, y, field_x, field_y, score_type='f1'):
        ...
        # print(len(true_y),end=' ')
        # print(len(predict_y))
        p = precision_score(true_y, predict_y, average='weighted')
        self.score = recall_score(true_y, predict_y, average='weighted')
        r = self.score
        acc = accuracy_score(true_y, predict_y)
        f1 = f1_score(true_y, predict_y, average='weighted')
        micro_f1 = f1_score(true_y, predict_y, average='micro')
        macro_f1 = f1_score(true_y, predict_y, average='macro')
        assert len(true_y) == len(predict_y)
        return metric_func(predict_y, true_y, average='micro'), p, r, acc, f1, micro_f1, macro_f1
