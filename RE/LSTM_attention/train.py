...
with open('./data/SemEval2010_task8/people_relation_test.pkl', 'rb') as inp:
    test = pickle.load(inp)
    labels_t = pickle.load(inp)
    position1_t = pickle.load(inp)
    position2_t = pickle.load(inp)
print("train len:{}       test len:{}       word2id len:{}".format(len(train), len(test), len(word2id)))
#    return word2id, id2word, relation2id, train, labels, position1, position2, test, labels_t, position1_t, position2_t
...
def train():
    for epoch in range(EPOCHS):
        ...
        for sentence, pos1, pos2, tag in train_dataloader:
            ...
            pos1 = Variable(pos1).to(DEVICE)
            pos2 = Variable(pos2).to(DEVICE)
            y = model(sentence, pos1, pos2)
            tags = Variable(tag).to(DEVICE)
